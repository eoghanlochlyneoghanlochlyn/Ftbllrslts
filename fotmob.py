...
def recursive_find(data, target_key):
    ...
    if isinstance(data, dict):
        ...
        for value in data.values():
            result = recursive_find(value, target_key)
        ...
        for item in data:
            result = recursive_find(item, target_key)
...
def find_section(data, possible_keys):
    """
    پیدا کردن اولین دیکشنری‌ای که یکی از کلیدهای موردنظر را دارد.
    """
    ...
    if isinstance(data, dict):
        ...
        for value in data.values():
            result = find_section(value, possible_keys)
        ...
        for item in data:
            result = find_section(item, possible_keys)
...
def fetch_match_page(match_id):
    ...
    print(
        f"FotMob {match_id}: Response length = {len(response.text)}"
    )
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            try:
                ...
                raise RuntimeError(
                    f"__NEXT_DATA__ JSON decode failed: {error}"
                )
...
def get_content(root):
    ...
    possible_paths = [
        ...
        ("props", "pageProps", "match", "content"),
        ("props", "pageProps", "matchData"),
    ...
    if isinstance(root, dict):
        ...
        if isinstance(props, dict):
            ...
            if isinstance(page_props, dict):
                print(
                    "FotMob pageProps sample keys:",
                    list(page_props.keys()),
                )

    # جست‌وجوی بازگشتی برای content
    recursive_content = recursive_find(
        root,
        "content",
    )

    if isinstance(recursive_content, dict):
        print(
    ...
    # بعضی ساختارها ممکن است مستقیماً matchData داشته باشند
    recursive_match_data = recursive_find(
        root,
        "matchData",
    )

    if isinstance(recursive_match_data, dict):
        print(
...
def extract_event_jsonld(root):
    possible_paths = [
        ("props", "pageProps", "seo", "eventJSONLD"),
        ("props", "pageProps", "eventJSONLD"),
        ("pageProps", "seo", "eventJSONLD"),
        ("seo", "eventJSONLD"),
    ]
...
def extract_basic_info(root):
    event_jsonld = extract_event_jsonld(root)
    ...
    if isinstance(event_jsonld, dict):
        home_team_data = event_jsonld.get("homeTeam")
        away_team_data = event_jsonld.get("awayTeam")
        ...
        match_name = clean_text(
            event_jsonld.get("name")
        )

    if not home_team:
        home_team = clean_text(
            recursive_find(content, "homeTeamName")
        )

    if not away_team:
        away_team = clean_text(
            recursive_find(content, "awayTeamName")
        )

    if not home_team:
        home_team = clean_text(
            recursive_find(root, "homeTeamName")
        )

    if not away_team:
        away_team = clean_text(
            recursive_find(root, "awayTeamName")
        )

    if not match_name and home_team and away_team:
...
def get_match_status(root):
    possible_paths = [
        ("props", "pageProps", "header", "status"),
        ("props", "pageProps", "status"),
        ("pageProps", "header", "status"),
        ("header", "status"),
    ]
...
def get_lineup(content, root=None):
    lineup = None
...
def get_match_snapshot(root):
    ...
    lineup_teams = get_lineup_teams(root)
